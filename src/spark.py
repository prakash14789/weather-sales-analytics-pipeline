...
# Apply Windows Hadoop workaround if HADOOP_HOME is not already configured
if sys.platform.startswith("win"):
    os.environ["HADOOP_HOME"] = os.environ.get("HADOOP_HOME", "C:\\hadoop")
    os.environ["PATH"] = "C:\\hadoop\\bin;" + os.environ["PATH"]

def get_spark_session(app_name="Holiday Analytics"):
    """
    ...
    return (
        ...
        .config("spark.jars", "drivers/postgresql-42.7.11.jar")
...
def run_feature_engineering(sales_csv_path="data/raw/csv/Sample - Superstore.csv", holidays_json_path="data/raw/api/holidays.json", weather_json_path="data/raw/api/weather.json", temp_output_dir="data/processed/final_analytics_temp", final_output_dir="data/processed/final_analytics"):
    """
    Executes the complete PySpark ETL and Feature Engineering pipeline.
    """
    spark = get_spark_session()

    # 1. Load raw sales data
    print(f"\nLoading sales data from {sales_csv_path}...")
    ...
    # 5. Load Holiday data
    print(f"Loading holiday data from {holidays_json_path}...")
    try:
        holiday_df = (
            spark.read
            .option("multiline", "true")
            .json(holidays_json_path)
        )
    ...
    print(f"Loading weather data from {weather_json_path}...")
    try:
        weather_raw_df = (
            spark.read
            .option("multiline", "true")
            .json(weather_json_path)
        )
    ...
    for f in glob.glob(os.path.join(final_output_dir, "*")):
        try:
            if os.path.isfile(f):
                os.remove(f)
    ...
    for f in glob.glob(os.path.join(temp_output_dir, "*")):
        try:
            ...
            print(f"Error: Could not move file {f} to {final_output_dir}: {e}")

    # Clean up temp directory
    try:
        shutil.rmtree(temp_output_dir)
    except Exception as e:
        print(f"Warning: Could not remove temp directory {temp_output_dir}: {e}")
